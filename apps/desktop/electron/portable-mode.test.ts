...
test('portable mode is enabled only for an absolute Windows launcher directory', () => {
  ...
  assert.deepEqual(
    resolvePortableMode({ env: { PORTABLE_EXECUTABLE_DIR: 'D:\\Hermes' }, platform: 'linux', pathModule: path.win32 }),
    { enabled: false }
  )
})
...
test('portable mode keeps desktop and Hermes state beside the launcher', () => {
  const mode = resolvePortableMode({
    env: { PORTABLE_EXECUTABLE_DIR: 'D:\\Tools\\Hermes' },
    platform: 'win32',
  ...
  assert.deepEqual(mode, {
    enabled: true,
    executableDir: 'D:\\Tools\\Hermes',
    dataDir: 'D:\\Tools\\Hermes\\data',
    hermesHome: 'D:\\Tools\\Hermes\\data\\hermes',
    userDataDir: 'D:\\Tools\\Hermes\\data\\Hermes'
  })
