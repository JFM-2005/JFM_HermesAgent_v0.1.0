...
/** JFM monochrome accent — ink black on white (Genshin-style chrome). */
const INK = '#1A1A1A'
const PAPER = '#FFFFFF'

const nousTint = (pct: number) => `color-mix(in srgb, ${INK} ${pct}%, ${PAPER})`
const nousTintTransparent = (pct: number) => `color-mix(in srgb, ${INK} ${pct}%, transparent)`

/**
 * Nous — JFM monochrome desktop identity. White surfaces, black borders and
 * primary actions (replaces the upstream Nous blue glass look).
 */
export const nousTheme: DesktopTheme = {
  name: 'nous',
  label: 'Nous',
  description: 'White surfaces with black borders and primary actions',
  colors: {
    background: PAPER,
    foreground: INK,
    card: PAPER,
    ...
    secondary: nousTint(6),
    secondaryForeground: INK,
    accent: nousTint(8),
    accentForeground: INK,
    border: nousTintTransparent(28),
    input: nousTintTransparent(32),
    ring: INK,
    midground: INK,
    composerRing: INK,
    destructive: '#C72E4D',
    ...
    sidebarBorder: nousTintTransparent(24),
    userBubble: nousTint(5),
    userBubbleBorder: nousTintTransparent(22)
  },
  darkColors: {
    background: '#0D2F86',
