...
param(
    ...
    [string]$Commit = "",
    [string]$Tag = "",
    [string]$RepoSlug = $(if ($env:HERMES_REPOSITORY) { $env:HERMES_REPOSITORY } else { "JFM-2005/JFM_HermesAgent_v0.1.0" }),
...
function Write-BootstrapMarker {
    ...
    $pinnedBranch = $Branch
    if (-not $pinnedBranch) {
        $pinnedBranch = "master"  # JFM fork default; install.ps1 -Branch default
    }
